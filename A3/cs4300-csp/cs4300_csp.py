...
def MRV_hueristic(domainlist):
    ...
    variable_list = list(domainlist.keys())
    TheMRV = variable_list[len(domainlist)-1]
    ...
    def backtrack(idx: int):
        ...
        for val in domains[v]:
            ...
            if consistent_with_local(v, assignment):
                # forward check Hueristic  
                #maybe create an function for forward check heuristic and the MRV and decides here
                pruned = []
                ok = True
...
def solve_backtracking_withMRV(csp: CSP, var_order: Optional[List[str]]=None) -> Iterable[Assignment]:
    domains = {v: list(ds) for v, ds in csp.domains.items()}
    order = var_order or list(domains.keys())
    cons_by_var: Dict[str, List[Constraint]] = {v: [] for v in domains}
    for c in csp.constraints:
